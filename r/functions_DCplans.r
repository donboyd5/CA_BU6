...
dcvalues <- function(dc_covered, w, b, p){
  ...
  dcvalues <- unnest(b, cols = c()) %>%
    mutate(dc_assets_boy=0,
           ...
           dc_invinc=0,
           dc_assets_eoy=0,
           dc_assets_atretire=0) # we want dc assets in retirement year minus 1 -- 0 in all other years 
  ...
  for(i in 2:nrow(dcvalues)){
    ...
    dcvalues$dc_assets_atretire[i] <- ifelse(b$age[i] == w$aor,
                                             dcvalues$dc_assets_eoy[i - 1],
                                             0)
  }
