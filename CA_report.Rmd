...
```{r ONETIME_bls_research_corr, eval=FALSE}
...
a <- proc.time()
df1 <- vroom(paste0(wpid, "cs/", "cs.data.1.AllData"), col_types = "cicnc")
b <- proc.time()
b - a # 31 secs
saveRDS(df1, paste0(wpid, "wpi_all.rds"))

df1 <- readRDS(paste0(wpid, "wpi_all.rds"))

# count(df1, year) # 2011-2019

df2 <- df1 %>%
  filter(str_sub(series_id, 1, 6)=="CSU00X",  # occupational detail
         str_sub(series_id, 13, 14)=="3O")  # datatype and case code -- 3"O" not zero
rm(df1)
gc(verbose=TRUE)
...
# I have NOT installed dev versions of vroom and reader but here is cood to do so
# devtools::install_dev("vroom")
#  or devtools::install_github("r-lib/vroom/vroom")
# devtools::install_github("tidyverse/readr")


# how to parse series_id
...
df4 <- df3 %>%
  rename(ocode=detail) %>%
  left_join(wpiown %>% 
              select(own=ownership_code, ownf=ownership_name),
            by="own") %>%
  left_join(wpioccs %>% 
  ...
  mutate(stfips=as.integer(stfips), 
         stabbr=case_when(stfips==00 ~ "US",
                          ...
                          TRUE ~ stabbr))
summary(df4)
count(df4, own, ownf)  # no federal government
count(df4, stfips, stname)  # 47 including all US, DC, GU, PR, VI (42 states)
count(df4, stfips, stabbr, stname)
count(df4, displevel)

tmp <- df4 %>%
  filter(stabbr=="CA", year>=2019)
  

glimpse(df4)
wpirates <- df4 %>%
  select(stabbr, year, own, ownf, ocode, oname, displevel, value)
summary(wpirates)
...
wpirates <- readRDS(here::here("data", "wpirates.rds")) # workplace injury rates -- DJB review
...
tab <- bind_rows(tibble(stname="California and the U.S.:"), tabdata) %>%
  ...
  cols_hide(stabbr) %>%
  tab_header(
    title = "Correctional officer wages, selected years"
      ) %>%
  cols_label(stname="",
             wage_2012="2012",
             wage_2016="2016",
...
# library(viridis)
# 
# sts <- c("CA", "AZ", "NV", "OR", "MA", "US")
# storder <- c("US", "CA", "MA", "NY", "OR", "NV", "AZ")
# storder <- c("CA", "MA", "NY", "OR", "NV", "AZ") # use stabbr for convenience
# pdata <- corr_oes %>%
#   filter(stabbr %in% storder, year %in% c(2012, 2016, 2020)) %>%
#   mutate(stabbr=factor(stabbr, levels=storder)) %>%
#   arrange(stabbr, year) %>%
#   mutate(stname=factor(pdata$stname, levels=unique(pdata$stname))) # forces same ordering as stabbr
# 
# 
# p <- pdata %>%
#   ggplot(aes(factor(year), rwage, fill=stname)) +
...
tab <- top10data %>%
  ...
  gt() %>%
  tab_header(
    title = html("Correctional officer wages<br>adjusted for state cost of living differences"),
...
#   cols_hide(stabbr) %>%
#   tab_header(
#     title = "Correctional officer wages adjusted for state cost of living differences",
#     subtitle= "Top 10 states by cost-of-living adjusted wage"
#       ) %>%
#   cols_label(stname="",
#              wage="Wage",
#              nrank="Rank",
#              pdwage="California % above",
#              rppwage="Wage",
#              rpprank="Rank",
#              pdrppwage="California % above") %>%
#   cols_align(align="left", columns = stname) %>%
#   tab_spanner(
#     label = html("Nominal wages"),
#     columns = c(wage, nrank, pdwage)
#     ) %>%
#   tab_spanner(
#     label = "Adjusted wages",
#     columns = c(rppwage, rpprank, pdrppwage)
#     )  %>%
#   fmt_number(
#     columns=c(wage, rppwage),
#     pattern = "${x}",
#     decimals=0
#   ) %>%
#   fmt_number(
...
#   
# corr_summary %>%
#   ggplot(aes(exp, ltassoc, colour=dumca)) +
#   geom_point() +
#   geom_hline(yintercept = median(corr_summary$ltassoc)) +
#   geom_vline(xintercept = median(corr_summary$exp))

# capt1 <- "Note: horizontal and vertical lines mark median"
# capt2 <- paste0("Source: ", source_acs, ".")
# capt <- paste0("\n", capt1, "\n", str_wrap(capt2, 100))
# 
# p <- corr_summary %>%
#   ggplot(aes(1 - ltassoc, exp, label=stabbr2, colour=dumca)) +
#   geom_point(size=0.275, colour="black") +
#   # geom_text(size=2.5, position = position_jitter()) +
#   # geom_text(size=2.5, nudge_x=-0.0075) +
#   geom_text(size=2.5, nudge_x=0.0015, hjust=0) +
#   geom_hline(yintercept = median(corr_summary$exp), size=0.1, colour="black") +
#   geom_vline(xintercept = median(1 - corr_summary$ltassoc), size=0.1, colour="black") +
#   scale_colour_manual(values=c("blue", "red")) +
#   scale_x_continuous(name="% of correctional officers with associate's degree or higher",
#                      labels = label_comma(scale=100, accuracy=1, suffix="%"),
#                      breaks=seq(.0, 1, .05)) +
#   scale_y_continuous(name="Estimated years of experience",
#                      breaks=10:30,
#                      labels = label_comma(accuracy=.1)) +
#   theme_bw() +
#   legend_none +
#   ggtitle("Education and experience of correctional officers by state") +
#   labs(caption=capt) +
#   caption_left
# p
# 
# ggsave(here::here("results", "state_educ_exp.png"), plot=p, width=10, height=8, scale=1.0)
# theme(axis.text.y = element_text(hjust = 0)) +
...
df %>% filter(ocode=="333012")

df2 <- df %>%
  filter(own==0 | (ocode=="333012" & own %in% c(1, 7, 8))) %>% # private, s, l, for corr officers, private other
  filter(displevel==4)

df2 <- df %>%
  filter(own %in% c(1, 7, 8)) %>% # private, s, l, for corr officers, private other
  filter(displevel==4) %>%
  mutate(ownf2=factor(own, levels=c(1, 7, 8), labels=c("private", "state gov't", "local gov't")),
         label=paste0(oname, " (", ownf2, ")"),
...
tabdata <- df2 %>%
  mutate(label=ifelse(ocode=="333012",
                      paste0(oname, " (", ownf, ")"),
                      oname),
         # label=str_replace_all(label, "[^[:alnum:]]", ""),    # Delete non-alphanumeric
         label=str_replace_all(label, "[^[:alnum:] (),]", ""),    # Delete non-alphanumeric
         # label=iconv(label, from = '', to = 'ASCII//TRANSLIT'), # convert to ascii
         rank=rank(desc(value))) %>%
  filter(rank <= 10 | ocode=="333012") %>%
  arrange(rank) %>%
  select(ocode, label, value, rank)
...
tab <- tabdata %>%
  ...
  cols_hide(ocode) %>%
  tab_header(
    title = "Nonfatal workplace injuries in the United States in 2019",
    subtitle = paste0("Top 10 occupations, plus correctional officers by sector, out of ",
                      noccs, 
                      " occupation-sector combinations")
      ) %>%
  tab_spanner(label=html("Injuries and illnesses requiring days away from work<br>Per 10,000 equivalent full-time equivalent workers"),
              columns=c(value, rank)) %>%
  cols_label(label="",
             value=html("Rate"),
             rank="Rank"
             ) %>%
...
tab
gtsave(tab, filename="tab_wpius.png", path=here::here("results"))
...
cacorr_fslacs <- caworkers %>%
  ...
  # no federal wpi rates, ONLY state and local
...
cacorr_fslacs %>%
  group_by(cowf) %>%
  summarise(n=n(), wpirate=mean(wpirate))

camod_fsl <- feols(lrppwagp ~ exp + exp2 + edattain + marstat + sex + rachisp + factor(year) | cowf,
               data=cacorr_fslacs, notes = FALSE)
summary(camod_fsl)

camod_sl_wpi <- feols(lrppwagp ~ exp + exp2 + edattain + marstat + sex + rachisp +
                        wpirate + factor(year) | cowf,
               data=cacorr_fslacs %>% filter(!is.na(wpirate)), notes = FALSE)
summary(camod_sl_wpi)
...
cfe <- tibble(cowf=names(fe$cowf), fe=fe$cowf) %>%
  mutate(cfe=fe - mean(fe),
         cfepct=exp(cfe) -1,
         state_vsother=(1 + cfepct[cowf=="state"]) / (1 + cfepct)) %>%
  arrange(desc(cfepct))
cfe

few <- fixef(camod_sl_wpi)
cfew <- few$cowf - mean(few$cowf)
# summary(few)

# centered fixed effects
cfew <- tibble(cowf=names(few$cowf), fe=few$cowf) %>%
  mutate(cfe=fe - mean(fe),
         cfepct=exp(cfe) -1,
         state_vsother=(1 + cfepct[cowf=="state"]) / (1 + cfepct)) %>%
  arrange(desc(cfepct))
...
```{r include=TRUE}
camod_cluster <- feols(lrppwagp ~ exp + exp2 + edattain + marstat + sex + rachisp + as.factor(year) | occgroup,
               data=cacluster_df)
...
df2 <- keyfacts$real_wages_oes %>%
  mutate(rank=rank(desc(rppwage)))
rppwageca <- keyfacts$corr_oes %>% filter(stabbr=="CA", year==2020) %>% .$rppwage
rppwagerec2 <- keyfacts$corr_oes %>% 
  filter(year==2020, stabbr %in% state.abb) %>% 
  arrange(-rppwage) %>%
  filter(row_number()==2)
rppwage2 <- rppwagerec2$rppwage
rppwagename2 <- get_stname(rppwagerec2$stabbr)
usrppwage <- keyfacts$corr_oes %>% filter(stabbr=="US", year==2020) %>% .$rppwage
...
-   Comparisons with state correctional officers in other states

    -   California state correctional officer wages are the highest in the nation. Before adjusting for cost of living differences, the average wage is r scales::percent(cawage / mawage - 1, accuracy=.1) higher than the next-highest state, Massachusetts, and r percent(cawage / uswage - 1, accuracy=.1) higher than the U.S.-average correctional officer wage.
    ...
    -   California has the highest cost of living in the nation. After adjusting for cost of living, California correctional officer wages are still the highest in the nation. They are r percent(rppwageca / rppwage2 -1, accuracy=.1) above the second-highest state, r rppwagename2, and r percent(rppwageca / usrppwage - 1, accuracy=.1) above the United States average.
    ...
    -   After adjusting for cost of living differences and controlling for education, experience, and demographic differences, California correctional officers' wages are about r percent(cacfe, .1) above the average state, and r percent(cacfe / cfe2 -1, .1) above the second-highest state, r cfename2. (This conclusion is based on [wage regressions](keystatefe) run on a sample of r comma(nobs(keyfacts$statemod)) correctional officers in the United States.)
    
    
```{r}
# wages within California
casg <- keyfacts$oesres_cafsl %>% filter(year==2020, level=="State") %>% .$amean
cafg <- keyfacts$oesres_cafsl %>% filter(year==2020, level=="Federal") %>% .$amean
calg <- keyfacts$oesres_cafsl %>% filter(year==2020, level=="Local") %>% .$amean
# (`r percent(casg/cafg - 1, .1)`) above the average federal government correctional officer wage, and `r comma(casg - calg)` (`r percent(casg/calg - 1, .1)`) above the average local government correctional officer wage.

# education and experience within California
casgacs <- keyfacts$cafsl_edexp %>% filter(cowf=="State")
cafgacs <- keyfacts$cafsl_edexp %>% filter(cowf=="Federal")
calgacs <- keyfacts$cafsl_edexp %>% filter(cowf=="Local")

cafg_fe <- keyfacts$fefsl_df %>% filter(level=="federal") %>% .$pdstate
calg_fe <- keyfacts$fefsl_df %>% filter(level=="local") %>% .$pdstate
  
```

-   Comparisons with correctional officers within California

    -   California state correctional officers wages have consistently been well above those of federal and local correctional officers in California. In 2020, according to the BLS OES research databsase, the average California state government correctional officer wage of r comma(casg, prefix="$") was r comma(casg - cafg, prefix="$") (r percent(casg/cafg - 1, .1)) above the average federal government correctional officer wage, and r comma(casg - calg, prefix="$") (r percent(casg/calg - 1, .1)) above the average local government correctional officer wage.
    -   According to the American Community Survey, California state correctional officers have:
        -   Less education than their local government counterparts (r number(casgacs$yoschool, .1) years vs. r number(calgacs$yoschool, .1) years on average), and
        -   More experience than both local government and federal government correctional officers in California (r number(casgacs$exp, .1) years vs. r number(calgacs$exp, .1) and r number(cafgacs$exp, .1) years on average).
    -   An econometric model of wages based on ACS data suggests that after controlling for education, experience, and demographic characteristics, state correctional officer wages are r percent(calg_fe, .1) above local wages, and r percent(cafg_fe, .1) above federal wages. This might best be considered a lower bound, because the ACS data, before controls, show smaller differences in wages than the OES data.


```{r}
# Comparisons to law etc.
# keyfacts$caclustercfe
corrsg_cfe <- keyfacts$caclustercfe %>% filter(str_detect(occname, "Correctional")) %>% .$cfe
detect_cfe <- keyfacts$caclustercfe %>% filter(str_detect(occname, "Detectives and criminal investigators")) %>% .$cfe

```

        
-   Comparisons with California private-sector workers in the Law, Public Safety, Corrections & Security job cluster

    -   State correctional officer wages, adjusted for intra-California regional price differences, after controlling for eduction, experience, and demographic characteristics, were higher than those of any private sector occupation in the Law, Public Safety, Corrections & Security job cluster.
    -   They were r percent(corrsg_cfe, .1) above the cluster average. The private sector occupation in the cluster with the highest wages, detectives and criminal investigators, had average wages that were r percent(detect_cfe, .1) above the cluster average.


```{r}
# keyfacts$caeduccfe 
corred_cfe <- keyfacts$caeduccfe %>% filter(str_detect(occname, "Correctional")) %>% .$cfe
locoed_cfe <- keyfacts$caeduccfe %>% filter(str_detect(occname, "Locomotive engineers and operators")) %>% .$cfe


```


-   Comparisons with California private-sector workers in occupations with similar education profiles as state correctional officers

    -   I defined "occupations with similar educational profiles" as those where the percentage of private-sector California workers in the ocupation was within +/- 5 percentage points of the percentage for California state government correctional workers (approximately 73%).
    -   I grouped together occupations that have fewer than 50 observations.
    -   State correctional officer wages, adjusted for intra-California regional price differences, after controlling for eduction, experience, and demographic characteristics, were higher than those of any private sector occupation in with similar educational attainment.
    -   They were r percent(corred_cfe, .1) above the group average. The private sector occupation in the group with the highest wages, locomotive engineers and operators, had average wages that were r percent(locoed_cfe, .1) above the group average.
