...
#.. ACS constants ----
acsvars_string <- "serialno, st, puma, powsp, powpuma, adjinc, pwgtp, agep, cow, mar, schl, sex, wagp, wkhp, wkw, wkwn, cit, hisp, rac1p, occp, naicsp"
...
sworker <- "wagp >= 10e3 & agep >= 18 & wkw <= 2 & wkhp >= 35"  # as a string - works with
...
# WKW Character 1 Weeks worked during past 12 months
...
# WKWN Numeric 2 Weeks worked during past 12 months (2019 or later)
# bb .N/A (less than 16 years old/did not work during the past .12 months or case is from before 2019)
# 1..52 .1 to 52 weeks worked during past 12 months (2019 or later

# eventually maybe keep workers with 35-70 usual hours per week, or something like that?
quantile(corr_workers$wkhp, probs = c(0, .01, .05, .1, .25, .5, .75, .9, .95, .99, 1))
quantile(corr_workers$wkw, probs = c(0, .01, .05, .1, .25, .5, .75, .9, .95, .99, 1))
quantile(corr_workers$wkwn, probs = c(0, .01, .05, .1, .25, .5, .75, .9, .95, .99, 1), na.rm=TRUE)
count(corr_workers, year, wkw) %>% pivot_wider(names_from = year, values_from = n)
count(corr_workers, year, wkwn) %>% pivot_wider(names_from = year, values_from = n) # only avail for 2019
# keep wkw <= 2
# keep wkhp >= 35
# add wkhp and bornus to regressions
count(corr_workers, cit)

corr_workers %>%
  group_by(stabbr) %>%
  summarise(n=n(), wtdn=sum(pwgtp), 
            wkhp_wmean=wtd.mean(wkhp, weights = pwgtp), 
            wkhp_median=median(wkhp), .groups="drop") %>%
  arrange(desc(wkhp_wmean))
...
casco_gcc1 <- gcc1 %>%
  ...
  select(Year, emptype=EmployerType, empname=EmployerName, dept=DepartmentOrSubdivision, Position,
...
### Nominal wages
```{r fig_wages_oes, include=TRUE, fig.width=10, fig.height=8}
...
ggsave(here::here("results", "fig_wages_oes.png"), plot=p, width=10, height=8, scale=1)
write_csv(pdata, here::here("results", "fig_wages_oes_data.csv"))
...
write_csv(tabdata, file=here::here("results", "tab_wages_states_oes_data.csv"))
...
```{r tab_rppwage_states_oes, include=TRUE, fig.width=6, fig.height=8}
top10data <- corr_oes %>%
  filter(year==2020) %>%
...
tab
gtsave(tab, filename="tab_rppwage_states_oes.png", path=here::here("results"), expand = 10)
write_csv(tabdata, file=here::here("results", "tab_rppwage_states_oes_data.csv"))
...
```{r fig_wages_acs, include=TRUE, fig.width=6, fig.height=8}
...
# ggsave(here::here("results", "fig_wages_acs.png"), plot=p, width=10, height=8, scale=1.25)
# write_csv(pdata, file=here::here("results", "fig_wages_acs_data.csv"))
...
```{r tab_educ_exp_acs, include=TRUE}
...
tab
gtsave(tab, filename="tab_educ_exp_acs.png", path=here::here("results"), expand = 10)
write_csv(tabdata, file=here::here("results", "tab_educ_exp_acs_data.csv"))

```

# Workplace injury tables

```{r tab_intentional_injuries_wpi}
...
gtsave(tab, filename="tab_intentional_injuries_wpi.png", path=here::here("results"), zoom=1, expand=10)
write_csv(tabdata, file=here::here("results", "tab_intentional_injuries_wpi_data.csv"))
...
# webshot options
# default zoom=2, expand=5
gtsave(tab, filename="tab_injuries_wpi.png", path=here::here("results"), zoom=1, expand=10)
write_csv(tabdata, file=here::here("results", "tab_injuries_wpi_data.csv"))
```
...
frmbase <- Formula(lrppwagp ~ exp + exp2 + edattain + wkhp +
                        marstat + sex +
                        bornus + rachisp + as.factor(year))
...
### Fixed effects model
```{r mod_statecos}
# summary(corr_workers_states) # don't worry about powpuma
# update syntax: first . is the first (and only) part of the lhs, 
#  second . is the rhs, which we update by adding | and the fe
modfe_states <- feols(update(frmbase, . ~ . | stabbr2),
               data=corr_workers_states)
summary(modfe_states)
fe_states <- fixef(modfe_states)
# etable(modfe_states)
# etable(modfe_states, tex = TRUE)
# library(latexpdf)
# as.pdf(tidy(modfe_states), stem = "don")
# as.pdf(etable(modfe_states, tex = TRUE) %>% as.document, stem="don")
# tmp <- tidy(modfe_states)
# as.pdf(as.data.frame(tmp))
# etable(modfe_states, postprocess.df = pandoc.table.return, style = "rmarkdown")
...
ggsave(here::here("results", "fig_model_statefe_bar.png"),
       plot=statefe_bar, width=10, height=8, scale=1.0)
write_csv(pdata, file=here::here("results", "fig_model_statefe_bar_data.csv"))
...
# update(frmbase, . ~ . | stabbr2)
moddum_states <- feols(update(frmbase, . ~ . + casco),
               data=corr_workers_states)
...
mod_cacorrfsl <- feols(update(frmbase, . ~ . | level),
               data=caworkers_corr)
...
```{r tab_model_fslpremium}

tabdata <- cfe_cacorrfsl %>%
  filter(level %in% c("local", "federal")) %>%
  select(level, casco_vsother) %>%
...
tab <- tabdata %>%
  ...
  cols_label(level="Employer",
             casco_vsother=html("Premium vs. other<br>level of government")
             ) %>%
  cols_align(align="left", columns = level) %>%
  fmt_percent(
    columns=casco_vsother,
    # pattern = "{x}",
    decimals=1
  ) %>%
  tab_source_note(paste0("Source: ", source_acs, ".")) %>%
  tab_footnote(
    "Adjusted for within-state cost-of-living differences and controlling for education, experience, and demographic characteristics.",
    locations = cells_column_labels(columns = casco_vsother)
    ) %>%
  cols_width(
    level ~ pct(40),
    everything() ~ pct(60)
...
# webshot options
# default zoom=2, expand=5
gtsave(tab, filename="tab_model_fslpremium.png", path=here::here("results"), zoom=2, expand=5)
write_csv(tabdata, file=here::here("results", "tab_model_fslpremium_data.csv"))
...
moddum_cacorrfsl <- feols(update(frmbase, . ~ . + casco),
               data=caworkers_corr)
...
### Fixed effects model
```{r mod_jobclust}
mod_calawjobs <- feols(update(frmbase, . ~ . | occgroup),
               data=caworkers_law)
...
```{r fig_model_lawcluster}
pdata <- cfe_calawjobs  %>%
  mutate(label=ifelse(row_number() <= 5, percent(cfepct, accuracy=.1), "")) %>%
  mutate(grp=ifelse(casco, "CA", "other")) %>%
  mutate(occgroup=ifelse(occgroup=="lowobs", "Low-observation occupations as a group", occgroup))

gtitle <- "California state correctional officer wages relative to jobs in the Law, Public Safety, Corrections & Security occupation cluster"
gsub1 <- "Controlling for experience, education, and demographic characteristics"
...
ggsave(here::here("results", "fig_model_lawcluster.png"), plot=lawclustfe_bar, width=10, height=8, scale=1.3)
write_csv(pdata, file=here::here("results", "fig_model_lawcluster_data.csv"))

```

### Dummy variable model
```{r}
moddum_calawjobs <- feols(update(frmbase, . ~ . + casco),
               data=caworkers_law)
...
occ_stats %>%
  ...
  fviz_nbclust(kmeans, method = "silhouette") +
    #geom_vline(xintercept = 4, linetype = 2) +
  labs(subtitle = "silhouette method")  # 2 or 4 looks good
...
#   labs(subtitle = "Gap statistic method") # 9 looks good
...
caworkers_cluster %>%
  ...
  group_by(casco) %>%
  summarise(n=n(), 
            across(c(rwagp, married, male, ltassoc, ltba, ltma, agep), 
                   ~ wtd.mean(.x, weights=pwgtp)),
            .groups="drop") %>%
...
### Fixed effects model
```{r mod_cluster}
mod_cacluster <- feols(update(frmbase, . ~ . | occgroup),
...
# webshot options
# default zoom=2, expand=5
gtsave(tab, filename="tab_model_jobcluster_top10.png",
       path=here::here("results"), zoom=2, expand=5)
write_csv(tabdata, here::here("results", "tab_model_jobcluster_top10_data.csv"))
```

```{r fig_model_jobcluster_bar}
...
caclusterfe_bar <- cfe_cacluster  %>%
  mutate(label=ifelse(row_number() <= 5, percent(cfepct, accuracy=.1), "")) %>%
  mutate(grp=ifelse(casco, "CA", "other")) %>%
  mutate(occgroup=ifelse(occgroup=="lowobs",
                         "Low-observation occupations as a group", 
                         occgroup)) %>%
  ggplot(aes(y=reorder(str_wrap(occgroup, 40), cfepct), x=cfepct)) +
  # ggplot(aes(y=reorder(occgroup, cfepct), x=cfepct)) +
  geom_col(aes(fill=grp), size=1.5, width=0.1) +
...
ggsave(here::here("results", "fig_model_jobcluster_bar.png"), 
       plot=caclusterfe_bar, width=10, height=12, scale=1.3)
write_csv(tabdata, here::here("results", "fig_model_jobcluster_bar_data.csv"))

```

### Dummy variable model
```{r}
moddum_cacluster <- feols(update(frmbase, . ~ . + casco),
               data=caworkers_cluster)
...
# webshot options
# default zoom=2, expand=5
gtsave(tab, filename="tab_models_premium_summary.png", 
       path=here::here("results"), zoom=1.5, expand=5)
write_csv(tabdata, here::here("results", "tab_models_premium_summary_data.csv"))

```


```{r fig_models_premium_summary_bar}
pdata <- tabdata %>%
  mutate(label=percent(casco_fepremium, accuracy=.1),
         compgroup=ifelse(vrosname=="states_vros",
                          "State correctional officers in other states", 
                          compgroup))

gtitle <- "California state correctional officer wage premium relative to comparison groups"
gsub1 <- "Controlling for experience, education, and demographic characteristics"
gsub2 <- "Wages adjusted for differences in cost of living"
gsubtitle <- paste0(gsub1, ". ", gsub2, ".")
...
models_premium_summary_bar <- pdata %>%
  ggplot(aes(x=reorder(str_wrap(compgroup, 40), desc(casco_fepremium)), y=casco_fepremium)) +
  # ggplot(aes(str_wrap(compgroup, 40), casco_fepremium)) +
  geom_col(fill="blue",  size=1, width=0.5) + #, size=1.5, width=0.1) +
  # geom_point(colour="blue", size=0.75) +
  geom_text(aes(label=label),
            hjust=0.5, nudge_y=.01, size=3.5) +
  scale_y_continuous(breaks=seq(0, 1, .1), labels=label_percent(accuracy = 1)) +
  theme_bw() +
  labs(y="% above group average, after controls", x=NULL,
       caption=capt) +
  ggtitle(label=gtitle, subtitle = gsubtitle) +
...
models_premium_summary_bar

ggsave(here::here("results", "fig_models_premium_summary_bar.png"), 
       plot=models_premium_summary_bar, width=12, height=8, scale=1.3)
write_csv(pdata, here::here("results", "fig_models_premium_summary_bar_data.csv"))
...
casco_premiums <- 
  cfe_states %>%
  filter(stabbr %in% c("MA", "OR", "TX")) %>%
  select(group=stabbr, ca_vsother) %>%
  bind_rows(cacluster_vros %>%
              mutate(group="private") %>%
              select(group, ca_vsother=cfepct))
casco_premiums
...
# base everything on cyor -- calendar year of retirement
cyor <- 2021
...
workers <- workers1 %>%
  ...
  select(worker, cyob, cyoe, cyor, cyod, aoe, aor, aod, fyos, fas)
workers
...
data <- 
  ...
  nest(wdata=c(cyob:fas)) %>%  # worker data
...
```{r}
# source functions ----
source(here::here("r", "functions_retirement_basic.r"))
...
# inflation constants
realwage_pch <-  0.009007175  # average real growth in Soc Sec wage index, 1950-ish to 2020
cpi_pch <- .02  # forecasted inflation rate
...
check <- data %>%
  ...
  mutate(base=list(fbase(wdata)),
         socsec=list(fsocsec(ss_covered, wdata, base, params, cpifc))) %>%
  ungroup()
...
# retinc
# glimpse(retinc)
# tmp <- retinc %>% unnest(cols=c(base, dcvalues)) %>% filter(worker==3)
```
...
retinc_pv <- retinc %>%
  unnest(cols = c(wdata, base, socsec, pension, dcvalues, params)) %>%
  ...
  mutate(across(.cols=c(db_eec, db_pension,
                        sstax, ssbenefit,
                        dc_eec, dc_annuity, dc_assets_atretire),
...
retinc_pv
glimpse(retinc_pv)
summary(retinc_pv) # make sure there are no NA values
ns(retinc_pv)

# tmp <- retinc_pv %>%
#   filter(db_pctfas != db_pctfas_uncapped)
# figure out when person retired!!! djb
...
results <- retinc_pv %>%
  ...
  summarise(fyos=first(fyos),
            ...
            dc_assets_atretire_pv=sum(dc_assets_atretire_pv), 
            .groups="drop") %>%
  mutate(net_penpv=db_pension_pv - db_eec_pv,
         net_sspv=ssbenefit_pv - sstax_pv,
         net_dcpv_alt=dc_annuity_pv - dc_eec_pv,
         net_dcpv=dc_assets_atretire_pv - dc_eec_pv,
...
## Examine results
```{r}
# retinc_pv has year by year details for each worker and all data and assumptions, so we can write it
# to an excel file for examination

# results has summary results (pvs) - use it to decide who to look at in detail
...
results %>%
  select(worker, stabbr, tier, tname, aoe, aor, aod, cyor, db_ipension, contains("net")) %>%
  arrange(worker, desc(net_retpv))
...
```{r makewb}
make_wb <- function(df, fnbase, comma_cols, pct_cols){
  wb <- createWorkbook()
  xlrows <- 2:(nrow(df) + 1)
  sname <- "sheet1"
  addWorksheet(wb, sname)
  writeData(wb, sheet=sname, x=df)
  
  # styles
  hrow <- createStyle(textDecoration = "bold", wrapText = TRUE) # header row
  ...
  icomma_cols <- match(comma_cols, names(df)) # indexes for the comma formatting
  ipct_cols <- match(pct_cols, names(df))

  # apply styles
  addStyle(wb, sheet = sname, style = hrow, rows=1, cols = 1:ncol(df), gridExpand = TRUE)
  ...
  saveWorkbook(wb, file = here::here("checks", paste0(fnbase, ".xlsx")), overwrite = TRUE)
}
...
# check results for individual workers or groups, export from retinc_pv --------------------------
w11 <- list(worker=1, stabbr="OR", tname="default")
w3dc <- list(worker=3, stabbr="US", tname="dc665")
w3pepra <- list(worker=3, stabbr="CA", tname="pepra")

# wcheck <- w3dc; fname <- "w3dc"
wcheck <- w3pepra; fname <- "w3pepra"
wfilter <- expression(worker==wcheck$worker & stabbr==wcheck$stabbr & tname==wcheck$tname)
check <- retinc_pv %>%
  filter(eval(wfilter))
results %>%
  filter(eval(wfilter))
...
vars <- quos(id, worker, stabbr, tier, tname, year, age,
             wage,
             db_eec, db_eec_pv,
             db_pension, db_pension_pv, 
             sstax, sstax_pv,
             ssbenefit, ssbenefit_pv,
             dc_eec, dc_eec_pv, dc_erc, dc_annuity, dc_annuity_pv, dc_assets_eoy,
             dc_assets_atretire, dc_assets_atretire_pv,
             db_benfactor_normal, fas, fyos, aoe, aor, aod, cyob)
# put them into groups for formatting
comma_cols <- quos(wage, db_eec, db_eec_pv,
             db_pension, db_pension_pv, 
             sstax, sstax_pv,
             ssbenefit, ssbenefit_pv, fas,
             dc_eec, dc_eec_pv, dc_erc, dc_annuity, dc_annuity_pv, dc_assets_eoy,
             dc_assets_atretire, dc_assets_atretire_pv) %>% unquos
pct_cols <- quos(db_benfactor_normal) %>% unquos

df <- check %>% 
  select(!!!vars)

make_wb(df, fname, comma_cols, pct_cols)

results %>%
  filter(eval(wfilter)) %>%
  pivot_longer(-c(worker, stabbr, tier, tname))
...
plans <- read_delim(
"stabbr; tname; plan
CA; poffa; California correctional officer (PO/FF A - hired prior to January 15, 2011)
CA; poffb; California correctional officer (PO/FF B - on or after January 15, 2011 and prior to January 1, 2013)
CA; pepra; California correctional officer new hire (PEPRA PO/FF - on or after January 1, 2013)
OR; default; Oregon correctional officer new hire
MA; default; Massachusetts correctional officer new hire
US; dc665; Private defined contribution (6% employer contribution, 6% employee contribution)
TX; default; Texas correctional officer new hire
", delim=";", trim_ws=TRUE
)
plans

tabdata <- results %>%
  filter(worker==3, # retires at age 57
         (tier!="private") | (tname=="dc665")) %>%  
  # CAUTION: dc_ror for public is always 5%
  left_join(plans, by=c("stabbr", "tname")) %>%
  select(worker, stabbr, tier, tname, plan, aoe, aor, aod, cyor, db_ipension, 
         net_penpv, net_sspv, net_dcpv, net_retpv) %>%
  arrange(worker, desc(net_retpv))

tabdata

tab <- tabdata %>%
  select(plan, db_ipension, 
         net_penpv, net_sspv, net_dcpv, net_retpv) %>%
  gt() %>%
  tab_header(
    title = "Present value of retirement income for an illustrative worker",
    subtitle = "Employee works ages 25 through 56, retires at 57, and lives until age 85. Final salary is $98,000 (approximate top of grade)."
      ) %>%
  tab_spanner(html("Present value at retirement (2021 $)<br>net of associated employee contributions"),
              columns=c(net_penpv, net_sspv, net_dcpv, net_retpv)) %>%
  cols_label(plan=html("Worker"),
             db_ipension=html("Pension annuity<br>in first year<br>of retirement"),
             net_penpv=html("Pension benefit"),
             net_sspv=html("Social<br>Security<br>benefit"),
             net_dcpv=html("Employer-supported<br>DC plan<br>(5% return)"),
             net_retpv=html("Total")
             ) %>%
  cols_align(align="left", columns = plan) %>%
  fmt_number(
    columns=c(db_ipension, net_penpv, net_sspv, net_dcpv, net_retpv),
    rows=1,
    pattern="${x}",
    decimals=0
  ) %>%
  ...
  tab_footnote(
    footnote = "Assets at retirement less present value of employee contributions.",
    locations = cells_column_labels(columns = net_dcpv)
  ) %>%
  tab_footnote(
    footnote = "Assumes worker has same salary history in each situation. If wages as California state correctional officer are higher than wages in other situations, California state correctional officer retirement income would be relatively higher than shown here.",
    locations = cells_title(groups = "subtitle")
  )
...
# webshot options
# default zoom=2, expand=5
gtsave(tab, filename="tab_pensions_npv.png", path=here::here("results"), zoom=1.5, expand=5)
write_csv(tabdata, here::here("results", "tab_pensions_npv_data.csv"))

```

```{r fig_pensions_npv_bar}

plans <- read_delim(
"stabbr; tname; plan
CA; poffa; California correctional officer (PO/FF A)
CA; poffb; California correctional officer (PO/FF B)
CA; pepra; California correctional officer new hire (PEPRA PO/FF)
OR; default; Oregon correctional officer new hire
MA; default; Massachusetts correctional officer new hire
US; dc665; Private defined contribution (6%--6% employer--employee contribution)
TX; default; Texas correctional officer new hire
", delim=";", trim_ws=TRUE
)
plans
...
pdata <- results %>%
  ...
  # CAUTION: dc_ror for public is always 5%
  left_join(plans, by=c("stabbr", "tname")) %>%
  select(worker, stabbr, tier, tname, plan, net_retpv) %>%
  arrange(worker, desc(net_retpv))
  # mutate(label=percent(casco_fepremium, accuracy=.1))

gtitle1 <- "Present value of retirement income for an illustrative worker"
gtitle2 <- "Pension income, Social Security income, and employer-supported DC assets, net of all employee contributions"
gtitle <- paste0(gtitle1, "\n", gtitle2)
gsubtitle <- "Employee works ages 25 through 56, retires at 57, and lives until age 85. Final salary is $98,000 (approximate top of grade)"
capt <- "NOTE: Assumes worker has same salary history in each situation. If wages as California state correctional officer are higher than wages in other situations, California state correctional officer retirement income would be relatively higher than shown here."

pensions_npv <- pdata %>%
  select(plan, net_retpv) %>%
  ggplot(aes(x=reorder(str_wrap(plan, 25), desc(net_retpv)), y=net_retpv)) +
  geom_col(fill="blue",  size=1, width=0.5) +
  scale_y_continuous(name="PV at retirement (thousands of 2021 $)",
                     breaks=seq(0, 3e6, 500e3), 
                     labels=label_comma(prefix = "$", accuracy = 1,
                     scale=1/1000)) +
  theme_bw() +
  labs(x=NULL, caption=capt) +
  ggtitle(label=gtitle, subtitle = gsubtitle) +
...
ggsave(here::here("results", "fig_pensions_npv_bar.png"), 
       plot=pensions_npv, width=12, height=8, scale=1.3)
write_csv(pdata, here::here("results", "fig_pensions_npv_bar_data.csv"))
...
```{r}
# gcc benefits ----
# REMEMBER - these are full time and part time so averages are understated
casco_gcc %>%
  group_by(position) %>%
  summarise(qtiledf(health))
...
# webshot options
# default zoom=2, expand=5
gtsave(tab, filename="tab_wagedata_compare.png", path=here::here("results"), zoom=1.5, expand=10)
write_csv(tabdata, here::here("results", "tab_wagedata_compare_data.csv"))
```
