...
For key conclusions see [the key conclusions section](#conclusions), or look for sections in the table of contents where the header includes "Key result" or "Key data".

There is an extensive appendix on data preparation that mostly serves as documentation.
...
gtitle <- "State correctional officer salaries in 2020"
gsubtitle <- NULL
...
capt1 <- paste0("\nSources: ", source_oesres, "; and ", source_rpp, ".")
capt <- str_wrap(capt1, 100)
pdata <- corr_oes %>%
  filter(year==2020, stabbr != "US") %>%
  mutate(stname=get_stname(stabbr),
         rwage=wage / rpp)
...
p <- pdata %>%
  ...
  scale_x_continuous(breaks=seq(0, 100e3, 10e3), labels=label_comma(accuracy=1, prefix="$")) +
  legend_none +
  caption_left
...
gsubtitle <- "Based on a 5-year sample adjusted to 2019 income levels"
capt1 <- paste0("\nSource: ", source_acs, ".")
capt <- capt1
pdata <- corr_wages %>%
  mutate(stname=get_stname(stabbr2),
...
#### Key result: differences across states (state fixed effects){#keystatefe}
...
mod_fsl <- feols(lrppwagp ~ exp + exp2 + edattain + marstat + sex + rachisp + as.factor(year) | cowf,
...
##### Key result: California state correction officers' wages relative to California private sector wages in the same cluster {#keycluster}
...
wpius <- wpirates %>%
  filter(stabbr=="US", year==2019, 
         (ocode=="333012" & own==7) | (ocode!="333012" & own==1)) %>%
...
Among occupations where workers have similar educational attainment, California state correctional officers have the highest rate of workplace injury resulting in days away from work.

```{r wpi_table, include=TRUE}

comps %>%
  mutate(oname_acs=ifelse(soc=="33-3012", paste0(oname_acs, " (State)"), oname_acs)) %>%
