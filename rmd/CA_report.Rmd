...
* What do relevant available data say about:
  ...
  *   How has their absolute and relative compensation changed since the last compensation study (prepared in 2013, released in 2015)?
  ...
  In doing this, adjust for regional cost differences to the extent practical. Pay attention to the fact that
salaries and benefits of new hires have changed in recent years.
...
sarpp <- sarpp1 %>%
  ...
  mutate(stfips=as.integer(stfips), year=as.integer(year), rpp=as.numeric(rpp)) %>%
...
For comparisons to the private sector, one approach is to compare against workers in jobs in a similar cluster.
See https://www.onetonline.org/find/career?c=12
...
## Comparisons of correctional officers' wages to other states
...
```{r include=FALSE}
# Number of correction officers per capita and average wage
pdata <- corr_oes %>%
  filter(year==2020) %>%
  left_join(spop.a %>%
              filter(year==2020) %>%
...
Here we look at raw wages, without adjusting for price differences across states or controlling for differences across workers such as educational attainment, as this is the starting point and is what other people will look at. (Because this does not adjust for cost-of-living differences across states, it overstates the relative cost of correctional officers in California.)

Still, it is important to see the raw data.

In addition, this is a sanity check on two different data sources:

*   Occupational Employment and Wage Statistics (OEWS or OES), research database: This is the gold standard for cross-state comparison in the number of workers or their average wages. But it is only one number per occupation per state per year (e.g., correctional officers in California in 2020 were paid $88,710 on average). It's a high quality number, but it's only one number: we don't have data for all of the workers comprising the number so we can't see how it varies within and across states with education, experience, etc.. (We do also have OES data for each MSA in California, from a variant of the OES.)
...
We don't expect summaries of the ACS to be as good a measure of overall average wages as what we see in the OES, but we'd like the two data sources to tell the same basic story. The two graps below suggest that they do.
...
### Econometric models of correctional officers' wages vs. states
...
## Comparisons of California state correctional officers' wages to California local and federal correctional officers
