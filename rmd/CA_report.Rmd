...
## Introduction to within-state comparisons
...
California state correctional officer wages vs. local and federal, after controls.
...
fefsl_df %>%
  ...
  gt() %>%
  tab_header(
    title = "California state correctional officer wages vs. those of federal and local"
      ) %>%
  cols_label(level="Level of government",
             pdstate="State CO wages % +/- other government") %>%
  fmt_percent(
    columns = c(pdstate),
    decimals = 1) 
...
Here we compare California *state* corrections officers' wages, adjusted for regional cost of living, with wages of California *private sector* workers in the [Law, Public Safety, Corrections & Security job cluster](https://www.onetonline.org/find/career?c=12&g=Go).
...
The following model regresses wages adjusted for regional cost of living differences on experience, education, and demographic characteristics. It has occupation fixed effects.
...
cacluster_df <- caworkers %>%
  ...
  mutate(n=n(),
         occgroup=ifelse(n <50, "lowobs", occname)) %>%
  ungroup
...
gtitle <- "California state correctional officer wages relative to private sector wages for jobs in the same cluster"
...
occfe_dots <- cfe %>%
  ...
  scale_colour_manual(values=c("blue", "red")) +
  theme_bw() +
  labs(x="% above or below cluster average", y=NULL,
       caption=capt) +
  ggtitle(label=gtitle, subtitle = gsubtitle) +
  theme(axis.text.y = element_text(hjust = 0)) +
  geom_vline(xintercept = 0) +
...
corred_workers_cut <- caworkers %>%
  ...
  group_by(occgroup) %>%
  mutate(occ_ltassoc=sum(ltassoc * pwgtp) / sum(pwgtp))
...
corred_workers_cut %>%
  ...
  summarise(nocc=first(nocc),
            ...
            .groups="drop")  # 0.7277604370	  # why not same??
