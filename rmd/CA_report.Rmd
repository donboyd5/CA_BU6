...
worker <- expression(wagp > 0, agep >= 18)
...
create_vars <- function(df){
  ...
  df %>%
    mutate(year=str_sub(serialno, 1, 4) %>% as.integer,
           ...
           edattain=factor(
             edattain,
             levels=c("notHSgrad", "HSgrad", "associate", "BA", "MAplus", "other")),
           marstat=factor(
             mar,
             levels=1:5,
...
sqlcmd <- paste0("SELECT ", acsvars_string, " FROM pus WHERE occp=3802") # 10 secs
system.time(df1 <- collect(tbl(db, sql(sqlcmd))))  # 3 secs
glimpse(df1)
count(df1, st)
...
corr_summary <- corr_wages %>%
  ...
  select(stabbr2, stname, n, wtdn, exp, rwagp, male, lehs) %>%
  arrange(stname)
...
corr_summary %>%
  ggplot(aes(exp, rwagp, label=stabbr2)) +
  geom_point(colour="blue", size=1) +
  geom_text(colour="blue", size=3) +
  theme_bw()
...
ggsave(here::here("results", "statefe_dots.png"), 
       plot=statefe_dots, width=10, height=8, scale=1)
