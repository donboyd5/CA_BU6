...
This section (results not shown):
...
*   Preparation of the two main ACS data subsets used in the analysis
...
#### Define ACS variables to use, and define a "worker"
...
*   Currently I define a worker as someone who:
  ...
  *   Graduated from high school or the equivalent (I may revisit this)
...
```{r acs_worker}
acsvars_string <- "serialno, st, puma, powsp, powpuma, adjinc, pwgtp, agep, cow, mar, schl, sex, wagp, hisp, rac1p, occp"
...
#### Collapse ACS race and ethnicity

*   Defines how to collapse race and ethnicity codes will 
...
#### Define key variables derived from variables on the ACS
...
#### Create an ACS data subset with all sampled state correctional workers in the U.S.
...
#### Create an ACS data subset with all sampled workers in California
...
caworkers <- cadf %>%
  ...
  left_join(carpp %>% 
              filter(year==2017) %>% 
              select(msacode=fips, rpp) %>%
              mutate(rpp=rpp / 100),
            by="msacode") %>%
  mutate(create_vars(.),
         msacode=ifelse(is.na(msacode), -9999, msacode),
         msaname=ifelse(is.na(msaname), "nonmatch", msaname),
         rpp=ifelse(msaname=="nonmatch", 1, rpp),
         rppwagp=rwagp / rpp,  # rwagp is real wage (adjusted for year), rppwagp is region adjusted
         lrppwagp=log(rppwagp))
summary(caworkers)
...
```{r corr_cutoff}
# number of observations needed in a state
corr_cut <- 50
```
...
## Examine California all occupations with ACS



### Where California corrections officers live, vs. other California workers
```{r}
# quick look at workers by puma
...
mod3 <- feols(lrwagp ~ edattain + exp + exp2 + sex + marstat + rachisp | soc + msaname, data=regdf)
# mod2 <- feols(lwage ~ edattain + exp + exp2 + female + married - 1 | soc, data=regdf)
